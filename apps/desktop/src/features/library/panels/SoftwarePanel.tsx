  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '10px', fontSize: '12px' }}>
        {record.version && (
          <div>
            <div style={{ color: 'var(--color-muted)', marginBottom: '2px' }}>Version</div>
        ...
        {record.architecture && (
          <div>
            <div style={{ color: 'var(--color-muted)', marginBottom: '2px' }}>Architecture</div>
      ...
      {record.purpose && (
        <div style={{ borderTop: '1px solid var(--color-border-subtle)', paddingTop: '8px', fontSize: '12px' }}>
          <div style={{ color: 'var(--color-muted)', marginBottom: '2px' }}>Purpose</div>
        ...
        <div style={{ borderTop: '1px solid var(--color-border-subtle)', paddingTop: '8px', fontSize: '12px' }}>
          {record.executable_path && (
            <div style={{ marginBottom: '6px' }}>
              <div style={{ color: 'var(--color-muted)', marginBottom: '2px' }}>Executable Path</div>
          ...
          {record.install_location && (
            <div>
              <div style={{ color: 'var(--color-muted)', marginBottom: '2px' }}>Install Location</div>
              <code style={{ fontSize: '11px', wordBreak: 'break-all', backgroundColor: 'var(--color-canvas)', padding: '2px 4px', borderRadius: 'var(--radius-sm)' }}>
                {record.install_location}
              </code>
            </div>
      ...
      {(record.installed_at || record.discovered_at) && (
        <div style={{ display: 'flex', gap: '16px', borderTop: '1px solid var(--color-border-subtle)', paddingTop: '8px', fontSize: '11px', color: 'var(--color-muted)' }}>
          {record.installed_at && <div>Installed: {record.installed_at.slice(0, 10)}</div>}
          {record.discovered_at && <div>Discovered: {record.discovered_at.slice(0, 10)}</div>}
        </div>
      )}
    </div>
  );
